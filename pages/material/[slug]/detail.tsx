...
function handleAccordionItemCondition({
  ...
  async function getNotesUrl(content: Content) {
    ...
    try {
      ...
      if (response.data.video_note_url && windowRef) {
        windowRef.location.href = response.data.video_note_url;
      } else {
        windowRef?.close();
        toast.error("Note video nggak ketemu");
      }
    } catch (error) {
      console.log(error);
      windowRef?.close();
  ...
  if (!loadingContents && (!contents || !contents.length)) {
    return (
      <div className="flex flex-col items-center justify-center gap-4 p-12">
        <Image
          src="https://serveproxy.com/?url=https://ruangobat.is3.cloudhost.id/statics/images/second-illustrations/confused.webp"
          priority
          width={300}
          height={300}
          className="h-32 w-32"
